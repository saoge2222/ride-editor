...
- **Slint source**: `crates/gui/src/main.slint`
- **Compilation**: `build.rs` calls `slint_build::compile("crates/gui/src/main.slint")`
- **Generated code**: consumed in `crates/main.rs` via `slint::include_modules!()`
...
Each UI component should reside in its own `.slint` file under `crates/gui/src/`, exporting a single component per file.
...
- Custom widgets should be placed in `crates/gui/src/` and imported in `main.slint`
