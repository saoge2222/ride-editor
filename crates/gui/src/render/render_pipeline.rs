...
pub(crate) fn record_draw(
    ...
    // SAFETY: the vertex buffer only contains geometry generated by this crate,
    // whose attributes match the bound pipeline; no shader can access memory or
    // descriptors outside those declared in the pipeline layout.
    unsafe {
        builder.draw(
            vertex_count,
            DRAW_INSTANCE_COUNT,
...
fn create_pipeline(
    ...
    let vertex_input_state = [Vertex2D::per_vertex()].definition(&vertex_entry)?;
